...
export const analyzeResumeController = async (req, res) => {
    ...
    // ⚠️ Make sure these are coming from auth middleware
    const userId = req.user?._id;
    const userEmail = req.user?.email;
    ...
    try {
        ...
        if (!resumeText.trim()) {
            return res.status(400).json({
                success: false,
                message: 'Could not extract text from the PDF. Please ensure it is not an image-only PDF.'
            });
        }

        const prompt = `You are an expert resume analyst. Your task is to evaluate a candidate's resume against a job description.
...
`;
        ...
        let geminiAnalysis = defaultResponse;
        ...
        try {
            ...
            if (
                ...
                try {
                    ...
                    geminiAnalysis = defaultResponse;
                }
            } else {
                console.error("Gemini response invalid:", JSON.stringify(result, null, 2));
            }
        ...
        const { extracted_skills = [], ats_score = 0 } = geminiAnalysis;
